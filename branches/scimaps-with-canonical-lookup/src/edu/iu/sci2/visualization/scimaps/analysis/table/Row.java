...
public class Row {
	...
	public <T> Row put(Column<T> column, T value) {
		...
		Preconditions.checkState(!values.containsKey(column),
				"Duplicate setting %s for column %s; value %s is already set.",
				value, column, values.get(column));
	 ...
	 *             that type in that column, or if the {@link Schema} implied by the values set for
	...
	public void copyIntoTuple(Tuple tuple) {
		Schema impliedSchema = Column.buildSchemaFor(values.keySet());
		Schema targetSchema = tuple.getSchema();
		
		// Check that the entry schema is >= the tuple schema (that we're not missing any values)  
		Preconditions.checkState(
				impliedSchema.isAssignableFrom(targetSchema),
				"The schema (%s) implied by this entry's values (%s) is not assignable from the target schema (%s).",
				impliedSchema, MAP_JOINER.join(values), targetSchema);
		
		// Check that the tuple schema is >= the entry schema (that we don't have any extra values))
		Preconditions.checkState(
				targetSchema.isAssignableFrom(impliedSchema),
				"The target schema (%s) is not assignable from the schema (%s) implied by this entry's values (%s).",
				targetSchema, impliedSchema, MAP_JOINER.join(values));
