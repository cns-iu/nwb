...
if [ "$EPIC_HOST" == "cns-epic-dev" ]; then
	...
	# For the production target, do not suppress restoration of privileges and ownership
	RESTORE_FLAGS=
else
...
restore_database_backup() {
	...
	echo "cd"                                                                                           >> $SCRIPT
	echo "pg_restore $RESTORE_FLAGS -h $DATABASE_HOST -U $DATABASE_USER -d epic_web \$DATABASE_ARCHIVE" >> $SCRIPT
	echo "echo   Done."                                                                                 >> $SCRIPT
