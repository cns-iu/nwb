...
//	}
	 ...
	 * Create a database and fill it with the provided parameters, and wrap and
	   return it in a nicely-packaged Data array.
	   header is an array of array-of-Strings-that-should-be-2-in-length, where the
	   first value is an database table column name, and the second value is the
	   type.
	 * contents is an array of array-of-Strings representing a table of values to be
	   placed in the database table.
	 * The length of header is the number of columns in the table, so it should
		...
		throws Exception
	{
		...
		try {
			DataSourceWithID database = databaseService.createDatabase();
			databaseConnection = database.getConnection();
			
			createEmptyDatabaseTable(tableName,
								database.getID(),
								header,
								primaryKey,
								databaseConnection);
			...
			fillDatabaseTable
				(tableName, database.getID(), header, contents, databaseConnection);

			Data databaseData =
				new BasicData(database, database.getClass().getName());
		...
		throws Exception
	{
		try {
			...
			Statement createAwardTableStatement =
	...
	private static void fillDatabaseTable(String tableName,
										  int databaseID,
										  String[][] header,
										  String[][] contents,
										  Connection databaseConnection)
		throws Exception
	{
		...
		String sqlTemplate = "INSERT INTO " + tableName + databaseID + " (" +
			columnNamesSQL.toString() + ") VALUES (" + valuesSQL.toString() + ")";
	
		PreparedStatement insertIntoAward =
			databaseConnection.prepareStatement(sqlTemplate);

		// Go through all rows in the table, placing the values into the prepared
		// statement, and adding the prepared statement to the batch of statements
