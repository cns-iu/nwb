...
public class MetadataReporterAlgorithm implements Algorithm {
    ...
    public Data[] execute() throws AlgorithmExecutionException {
    	...
    	try {
    		...
    		metadataWriter = new FileWriter(bundleNameFile);

    		metadataWriter.append(String.format(
    			"Menu Label,Algorithm ID,Containing Plug-In,Description%n"));

    		for (ServiceReference serviceReference : serviceReferences) {
    			Object menuPathObject = serviceReference.getProperty("label");

    			if (menuPathObject != null) {
    				String menuLabel = menuPathObject.toString();
    				String algorithmPID = serviceReference.getProperty("service.pid").toString();
    				String containingPluginName = serviceReference.getBundle().getSymbolicName();
    				Object descriptionObject = serviceReference.getProperty("description");
    				...
    				String row = String.format(
    					...
    					menuLabel,
    					algorithmPID,
    					containingPluginName,
    					description);
    				metadataWriter.append(row);
    			}
    		}

    		return formOutData(bundleNameFile);
...
//    			this.bundleContext.getAllServiceReferences(null, null);
//
//    		Multimap<String, String> symbolicNamesToReport =
...
//
//    		for (ServiceReference serviceReference : serviceReferences) {
////    			System.err.println("property keys: " + StringUtilities.implodeStringArray(
...
//    				symbolicNamesToReport.put(symbolicName, pluginString);
//
...
//
//    		for (Iterator<String> keys = symbolicNamesToReport.keySet().iterator();
//    				keys.hasNext();) {
//    			String key = keys.next();
//
//    			for (String value : symbolicNamesToReport.get(key)) {
...
//    }
    ...
    private static Data[] formOutData(File bundleNameFile) {
		Data reportFileData = new BasicData(bundleNameFile, CSV_MIME_TYPE);
		Dictionary<String, Object> reportFileMetadata = reportFileData.getMetadata();
		reportFileMetadata.put(DataProperty.LABEL, "Algorithm Metadata Report");
		reportFileMetadata.put(DataProperty.TYPE, DataProperty.TABLE_TYPE);
    	
        return new Data[] { reportFileData };
    }
