package edu.iu.sci2.database.scopus.load.tests;
...
import edu.iu.sci2.database.scopus.load.ScopusDatabaseLoaderAlgorithmFactory;
...
public class TryIntegrationTest {
	...
	public static void ensureConnection() throws Exception {
		if (connection == null) {
			...
			ScopusDatabaseLoaderAlgorithmFactory factory = new ScopusDatabaseLoaderAlgorithmFactory();
			CIShellContext context = TestUtilities.createFakeCIShellContext();
			
			String file =
					FileUtilities.safeLoadFileFromClasspath(RowItemTest.class, 
							"/edu/iu/sci2/database/scopus/load/testdata/BrainCancer.scopus").toString();
	
			Data inFile = new BasicData(file, "file:text/csv");
			
			Algorithm algo = factory.createAlgorithm(new Data[] {inFile}, new Hashtable(), context);
