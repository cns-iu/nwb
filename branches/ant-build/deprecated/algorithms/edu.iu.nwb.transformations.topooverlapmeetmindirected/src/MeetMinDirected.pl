...
#	print STDERR "$kjh nodes processed -> Direction\n";
	...
	foreach $node2 (keys %F1)
	{
		...
		if ($k1 < $k2)
		{
		$DirectedOverlap_i_p{$node1}{$node2}=$DirectedOverlap_i_p{$node1}{$node2}/($k1);	
		}
		else
		{
		$DirectedOverlap_i_p{$node1}{$node2}=$DirectedOverlap_i_p{$node1}{$node2}/($k2);
		}	
...
#######################################################################
#########################################################################
##### Calculate Topological Overlap for Directed Case for Promoters##
#########################################################################
##########################################################################

#print "calculating overlap between promoters ...... !!!!\n";
$kjh=0;
$sss=0;
foreach $node1 (keys %Promoters)
{
	$kjh+=1;
	$sss+=1;
	if ($sss==100)
...
#	print STDERR "$kjh nodes processed <- Direction\n";
	...
	@Node1Friends = (keys % {$Directed_p_i{$node1}});	
	#print "...................... $node1 Dude1 has as Friends @Node1Friends\n";
	...
	foreach $node2 (keys %F1)
	{
		@Node2Friends = (keys % {$Directed_p_i{$node2}});
	#print "$node2 Dude 2 has as Friends @Node2Friends\n";			
		########## find the overlap between the dudes ##############
		...
		foreach $f1 (@Node1Friends)
		{
			...
			{	
				if ($f1 eq $f2)
				{
				$DirectedOverlap_p_i{$node1}{$node2} += 1;
				$marcador=1;
				}		
			}	
		}
		...
		if ($marcador==0)
		{
			$DirectedOverlap_p_i{$node1}{$node2} = 0;
		}
		...
		if ($k1 < $k2)
		{
		$DirectedOverlap_p_i{$node1}{$node2}=$DirectedOverlap_p_i{$node1}{$node2}/($k1);	
		}
		else
		{
		$DirectedOverlap_p_i{$node1}{$node2}=$DirectedOverlap_p_i{$node1}{$node2}/($k2);
		}	
		#print "and their overlap is $DirectedOverlap_p_i{$node1}{$node2} \n";
	}	
...
foreach $n (sort {$a <=> $b} (keys %NodeName))
{
		if (exists($NumAttributes{$n}))
		{
			...
			$Amax=scalar(keys %NodeAttributes);
...
############################ PRINT OUTPUT ############ 222222222222 ###########################

print OUTPUTtofrom "//Calculated by Cesifoti.... :-) .... NWB\n"; 
print OUTPUTtofrom "*Nodes $NumberOfNodes\n";
...
foreach $n (sort {$a <=> $b} (keys %NodeName))
{
		if (exists($NumAttributes{$n}))
		{
			print OUTPUTtofrom "$n $NodeName{$n}"; 
			$Amax=scalar(keys %NodeAttributes);
			for ($r=0;$r<$NumAttributes{$n}-2;$r++)
			{
			print OUTPUTtofrom " $NodeAttributes{$n}{$r}";
			}
			print OUTPUTtofrom "\n";
			$C+=1;
		}
...
print OUTPUTtofrom "*DirectedEdges\n";
	foreach $node1 (sort(keys %DirectedOverlap_p_i))
	{
		foreach $node2 (sort(keys % {$DirectedOverlap_p_i{$node1}}))
		{
			if ($DirectedOverlap_p_i{$node1}{$node2} eq 0)
			{
			next;
			}	
			else
			{
			print OUTPUTtofrom "$node1 $node2 $DirectedOverlap_p_i{$node1}{$node2}\n";
			}
		}
	}
