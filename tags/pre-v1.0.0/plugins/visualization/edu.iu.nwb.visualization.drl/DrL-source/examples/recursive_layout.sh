...
do
  ...
  # now we use the single link clustering algorithm
  if [ $LEVEL -eq 1 ]
  then
    echo $BINDIR"/single_link" $ROOTNAME
    $BINDIR/single_link $ROOTNAME
    ...
    echo $BINDIR"/single_link" $ROOTNAME"_"$LEVEL
    $BINDIR/single_link $ROOTNAME"_"$LEVEL
