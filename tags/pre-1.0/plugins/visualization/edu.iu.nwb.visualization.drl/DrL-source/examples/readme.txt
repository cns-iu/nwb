...
There are a few different ways to run DrL, described in the 
...
Previous clustering work related to DrL using this dataset has appeared 
...
Serial
------
First, you can run it as a DrL serial program without recursion.  In 
...
matrix for a weighted graph.  An example of DrL in this case is given in 
the serial directory.  To run serial non-recursive DrL go to the serial 
directory and type
...
Recursive
---------
An example showing how to run DrL using the recursive option is found in 
...
5. To look at the results you can examine the following files:
   ...
   .clust -- the single link cluster files generated during coarsening
...
version of DrL.  Here are a few tips on adjusting the parameters to get 
desired effects.
...
parameters in the layout program.  In the recursive version of DrL, 
...
Parallel
--------
The parallel version of DrL can be run either with or without the 
...
You can also run DrL in the full Distributed recursive Layout mode.  
...
Tips for Using the Parallel Version of DrL
------------------------------------------
1. The parallel version of DrL is used only for extremely large graphs.  
If your computer has a lot of memory it is better to use the serial 
recursive version.  In any case, the only truly parallel code in the DrL 
...
Real-Time Option
----------------
The last feature that might be of interest in DrL is the real-time 
