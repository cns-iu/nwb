...
TEST_TAG_CONTENTS_WITH_JAVASCRIPT = \
    'javascript:%s' % TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT
...
class TagBaseTestCase(CustomTestCase):
    ...
    def testOpen(self):
        open_result = self.tag_base.open(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
        
        self.assertEqual(open_result, '')
    
    def testClose(self):
        close_result = self.tag_base.open(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
...
class SimpleTagTestCase(CustomTestCase):
    ...
    def testOpen(self):
        open_result = self.simple_tag.open(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
    ...
    def testClose(self):
        close_result = self.simple_tag.close(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
...
class LinkTagTestCase(CustomTestCase):
    ...
    def testOpenWithoutJavaScript(self):
        open_result = self.link_tag.open(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
        expected_open_result = \
            '<a href="%s">' % TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT
        
        self.assertEqual(open_result, expected_open_result)
    
    def testOpenWithJavaScript(self):
        open_result = self.link_tag.open(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITH_JAVASCRIPT)
        expected_open_result = \
            '<a href="%s">' % TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT
    ...
    def testClose(self):
        close_result = self.link_tag.close(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
...
class ImageTagTestCase(CustomTestCase):
    ...
    def testOpenWithoutJavaScript(self):
        open_result = self.image_tag.open(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
        expected_open_result = \
            '<img src="%s" />' % TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT
    ...
    def testClose(self):
        close_result = self.image_tag.close(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
...
class QuoteTagTestCase(CustomTestCase):
    ...
    def testOpen(self):
        open_result = self.quote_tag.open(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
        expected_open_result = '<blockquote><em>%s</em><br />' % \
            TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT
        
        self.assertEqual(open_result, expected_open_result)
    
    def testClose(self):
        close_result = self.quote_tag.close(
            TEST_TAG_NAME, TEST_TAG_CONTENTS_WITHOUT_JAVASCRIPT)
...
class PostMarkupTestCase(CustomTestCase):
    ...
    def testDefaultTags(self):
        ...
        self.failIfNot(_verifyPostMarkupHasTags(
            self.post_markup, include_tags=self.default_tags))
    
    def testRenderDirtyInputToHTML(self):
        rendered_html = \
            self.post_markup.render_to_html(self.DIRTY_INPUT)
