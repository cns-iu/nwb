...
ofstream outputFile("communities.tree");
...
Community::partition2graph() {
  ...
  for (int i=0 ; i<size ; i++) {
    ...
    for (int j=0 ; j<deg ; j++) {
      int neigh = *(p.first+j);
      outputFile << renumber[n2c[i]] << " " << renumber[n2c[neigh]] << endl;
    }
  }
}
...
Community::display_partition() {
  ...
  for (int i=0 ; i<size ; i++)
    outputFile << i << " " << renumber[n2c[i]] << endl;
}
...
Community::partition2graph_binary() {
  ...
  g2.links = new int[links.size()];
  std::copy(links.begin(), links.end(), g2.links);
  links.clear();

  g2.weights = new int[weights.size()];
  std::copy(weights.begin(), weights.end(), g2.weights);
  weights.clear();
