  ...
  if(argc > 2)
    {
      ...
      sprintf(quantityname, "degree_averagestrength.linbinned");
      sprintf(output_type, "plot");
      allocateMemoryForGlobalMeasurementLinearBinned(minnodedegree, maxnodedegree);
      determineAverageQuantityWithRespectToDegreeLinearBinned(averagenodestrength, nodedegreedistribution, nodestrength, 
							      minnodedegree, maxnodedegree, nodedegree, numberofnodes);
      writeAverageQuantityWithRespectToDegreeLinearBinned(averagenodestrength, nodedegreedistribution, minnodedegree, maxnodedegree, 
							  quantityname, output_type);

      sprintf(quantityname, "degree_averagestrength.logbinned");
      sprintf(output_type, "plot");
      allocateMemoryForGlobalMeasurementLogarithmicBinned(numberofbins);
      determineAverageQuantityWithRespectToDegreeLogarithmicBinned(averagenodestrength_logbinned, nodedegreedistribution_logbinned, newnodedegree_logbinned, 
								   nodestrength, minnodedegree, maxnodedegree, nodedegree, numberofnodes, numberofbins);
      writeAverageQuantityWithRespectToDegreeLogarithmicBinned(averagenodestrength_logbinned, nodedegreedistribution_logbinned, newnodedegree_logbinned, 
...
void allocateMemoryForGlobalMeasurementLinearBinned(int min, int max)
{
  ...
  averagenodestrength=(double *) calloc(max-min+1, sizeof(double));
}

void freeMemoryOfGlobalMeasurementLinearBinned(void)
{
  free(nodedegreedistribution);
  free(averagenodestrength);
}
...
void allocateMemoryForGlobalMeasurementLogarithmicBinned(int nbin)
{
  ...
  averagenodestrength_logbinned=(double *) calloc(nbin, sizeof(double));
}
...
void freeMemoryOfGlobalMeasurementLogarithmicBinned(void)
{
  ...
  free(averagenodestrength_logbinned);
}
