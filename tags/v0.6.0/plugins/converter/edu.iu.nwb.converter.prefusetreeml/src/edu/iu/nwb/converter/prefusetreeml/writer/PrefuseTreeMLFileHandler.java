...
public class PrefuseTreeMLFileHandler implements AlgorithmFactory {
    ...
    public class PrefuseGraphMLFileHandlerAlg implements Algorithm {
        ...
        public Data[] execute() {
        	...
        	if(inData instanceof File && format.equals("file:text/treeml+xml")){
        		...
        		} else if (!format.equals("file:text/treeml+xml")) {
        			logger.log(LogService.LOG_ERROR, "Expect file:text/treeml+xml, but the input format is "+format);
        		}
       			return null;
        	}     	
