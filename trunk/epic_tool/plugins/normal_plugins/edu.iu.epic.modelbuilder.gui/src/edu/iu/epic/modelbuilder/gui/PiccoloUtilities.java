...
public class PiccoloUtilities {
	...
	public static boolean isTransitionDuplicate(PNode sourceCompartment,
		...
		if (((String) sourceCompartment.getAttribute(GlobalConstants.NODE_ID_ATTRIBUTE_NAME))
				.equalsIgnoreCase((String) targetCompartment.getAttribute(GlobalConstants.NODE_ID_ATTRIBUTE_NAME))) {
		...
		for (Iterator transitionIterator = transitionLayer.getChildrenIterator();
			...
			List<PNode> compartmentsInvolved = (ArrayList<PNode>) transition
													.getAttribute(GlobalConstants
															.TRANSITION_INVOLVED_COMPARTMENTS_ATTRIBUTE_NAME); 
			
			String newTransitionSourceCompartmentID = (String) sourceCompartment
															.getAttribute(GlobalConstants.NODE_ID_ATTRIBUTE_NAME);
			
			String currentTransitionSourceComprtmentID = (String) compartmentsInvolved.get(0)
															.getAttribute(GlobalConstants.NODE_ID_ATTRIBUTE_NAME);
			
			String newTransitionTargetCompartmentID = (String) targetCompartment
															.getAttribute(GlobalConstants.NODE_ID_ATTRIBUTE_NAME);
			
			String currentTransitionTargetCompartmentID = (String) compartmentsInvolved.get(1)
															.getAttribute(GlobalConstants.NODE_ID_ATTRIBUTE_NAME);
			
			if (newTransitionSourceCompartmentID.equalsIgnoreCase(currentTransitionSourceComprtmentID)
					&& newTransitionTargetCompartmentID.equalsIgnoreCase(currentTransitionTargetCompartmentID)) {
