 ...
 */
uint64_t input_file::AToLong(string temp) {
	uint64_t number = 0;
...
map<string, string> input_file::read_input(string filename, string c_char) {
	...
	while (!in.eof()) {
		...
		if (input.substr(0, c_char.length()) != c_char) {
			...
			string firstTokenOfValue;
			iss >> firstTokenOfValue;

			string remainder = input.substr(iss.tellg(), input.length());
			firstTokenOfValue.append(remainder);			
			value = firstTokenOfValue;		
...
void input_file::input(string filename, unsigned argc, char **argv,
		string c_char) {
	unsigned i = 1;
	...
	header.push_back(
			pad(
					"# These options override the input file variables, where applicable"));
	header.push_back(separator());
}

input_file::input_file(unsigned argc, char **argv, char **env, string us,
		string c_char) {
	if (us == "") {
		us = "Correct usage:\n\n";
		us += string(argv[0]);
...
FILE *input_file::openFile(string filename, bool verbose) {
	...
	if (fp == NULL) {
		cout << "FILE *input_file::openFile(string filename, bool verbose)" << endl;
		fprintf(stderr, "Error opening file: %s\n", filename.c_str());
		exit(-1);
	}
...
FILE *input_file::readFile(string filename, bool verbose, char c_char) {
	...
	if (filename.find(".gz") != string::npos)
		...
		if (fp == NULL) {
			cout << "FILE *input_file::readFile(string filename, bool verbose, char c_char)" << endl;
			fprintf(stderr, "Error opening file: %s\n", filename.c_str());
			exit(-1);
		}
 ...
 */
uint64_t input_file::getLongOpt(string which) {
	if (vars.find(which) == vars.end())
		printUsage(which);

	return AToLong(Evaluate(vars[which.c_str()]));
}
 ...
 */
uint64_t input_file::getLongOpt(string which, uint64_t value) {
	if (vars.find(which) == vars.end())
		return value;
...
void input_file::writeToFile(FILE *fp) {
	...
	if (fp == NULL) {
		fprintf(stderr,
				"# Error opening file for output. Invalid file pointer provided\n");
		return;
	}
...
void input_file::writeToFile(FILE *fp, std::string filename) {
	...
	fprintf(fp, "\n");
	fprintf(fp, separator().c_str());
	fprintf(fp, "\n");
	fprintf(fp, empty_line().c_str());
	fprintf(fp, "\n");
	fprintf(fp, (pad("# Contents of " + filename) + "\n").c_str());
	fprintf(fp, empty_line().c_str());
	fprintf(fp, "\n");
	fprintf(fp, separator().c_str());
	fprintf(fp, "\n");
