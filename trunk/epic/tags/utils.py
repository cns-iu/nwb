...
def _calculate_tag_cloud_weight_min_and_max(tags):
	"""
	>>> import datetime
	>>> date1 = datetime.date(1984, 3, 15)
	>>> date1
	datetime.date(1984, 3, 15)
	>>> date2 = datetime.date(1985, 11, 19)
	>>> date2
	datetime.date(1985, 11, 19)
	>>> date3 = datetime.date(1985, 9, 24)
	>>> date3
	datetime.date(1985, 9, 24)
	>>> from tags.common import Tag
	>>> tag1 = Tag('tag1', date1, 1)
	>>> tag2 = Tag('tag2', date2, 5)
	>>> tag3 = Tag('tag3', date3, 50)
	>>> tags = [tag3, tag1, tag2]
...
def _annotate_tag_with_font_size(original_tag, tag_weight, thresholds):
	...
	annotated_tag = Tag(original_tag.tag_name,
