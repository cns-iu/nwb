...
TagCloud.Tag.prototype.toElement = function(isLastTag)
{
	...
	linkElement.setAttribute('id', this.name);
	linkElement.setAttribute('href', this.url);
	
	var tagDisplayText = this.name;
	
	if (tagDisplayText.length >= 12)
		tagDisplayText = tagDisplayText.substring(0, 11) + "\u2026";
...
TagCloud.Effector.CountSize.prototype.affect = function (tags)
{
	...
	if (minFontSize < this.baseFontSize)
		minFontSize = this.baseFontSize;
