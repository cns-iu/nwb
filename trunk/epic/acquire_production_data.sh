...
echo "pg_dump -h cns-dbp -U epic_dumper --schema=public -Fc epic_web > /tmp/dumped.dump" >> remote.sh
echo "pg_restore -l /tmp/dumped.dump" >> remote.sh
echo "pg_dump -h cns-dbp -U epic_dumper --schema=public -Fc epic_web | pg_restore -r --no-acl --no-owner -h $DATABASE_HOST -U epic_appuser -d epic_web" >> remote.sh
