...
def show(mesh):
    return 'gsave /Garamond findfont 30 scalefont setfont 0.1 setgray 30 50 moveto (' + mesh + ') show grestore'
...
def main():
    ...
    if columns < rows:
        ...
        columns = rows
        rows = temp
    yscale = 500.0 / (2 * RADIUS * int(rows))
    xscale = 700.0 / (2 * RADIUS * int(columns))
    print xscale, yscale
